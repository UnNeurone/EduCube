...
class EduCubePlantEnv(gym.Env, EzPickle):
    ...
    def __init__(self):
      ...
      self.observation_space = (VIEWPORT_W,VIEWPORT_H)#spaces.Box(0, np.inf, shape=(self.nb_obs_state,), dtype=np.float32)
      self.previous_state = []
...
# self.moon.CreateEdgeFixture(vertices=[p1,p2],density=0,friction=0.1)
    ...
    def step(self, action):
      ...
      for i in range (0,len(self.plantlist)):
        ...
        if self.viewer != None:
          temp_image = self.viewer.get_array()
          temp_image = np.moveaxis(temp_image, -1, 0)
          rgb_weights = [0.2989, 0.5870, 0.1140]
          temp_image = [np.dot(self.viewer.get_array()[..., :3], rgb_weights)]
          #rgb_weights = [0.2989, 0.5870, 0.1140]
          #grayscale_image = np.dot(self.viewer.get_array()[..., :3], rgb_weights)
        else:
          temp_image = np.zeros((1, VIEWPORT_H, VIEWPORT_W), np.uint8)  # np.zeros((VIEWPORT_W,VIEWPORT_H))
          #temp_image = np.zeros((3,VIEWPORT_H,VIEWPORT_W), np.uint8)#np.zeros((VIEWPORT_W,VIEWPORT_H))
          #print(grayscale_image.shape)

        if len(self.previous_state)!=0:
          if alive == 0:
            reward += size - self.previous_state[i][0]
            reward += age
          else :
            reward -= 100
            temp_done+=1
      ...
      return np.array(temp_image, dtype=np.float32), reward, done, {}
...
if __name__ == '__main__':
        ...
        for episode in tqdm(range(50)):
          ...
          for _ in range(200):
            env.render()
            #print(env.getSize())
            obs, reward, done, info = env.step(env.action_space.sample().clip(0,0.5))  # take a random action
            #time.sleep(0.2)
