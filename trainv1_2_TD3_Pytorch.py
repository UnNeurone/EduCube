...
from EduCubePlant_env import EduCubePlantEnv
from random import shuffle


class Critic(nn.Module):
    def __init__(self, Q_layer, nb_obs, nb_action, input ):
...
class Actor(nn.Module):
    def __init__(self, mu_layer, nb_obs, nb_action, input):
...
class TD3Agent:
    def __init__(self, env='LunarLanderContinuous-v2', input = 'state',mu_layer=[400, 300], Q_layer=[400, 300],
        ...
        self.critic_1 = Critic(Q_layer, self.nb_obs, self.nb_action , self.input )
        self.critic_opt_1 = optim.Adam(self.critic_1.parameters(),lr = Ad_alpha_Q, weight_decay=l2_weight_decay_Q)
        #Random initialization of the Critic Network
        self.critic_2 = Critic(Q_layer, self.nb_obs, self.nb_action,  self.input )
        ...
        self.actor = Actor(mu_layer, self.nb_obs, self.nb_action, self.input )
        ...
        self.mu_prim = Actor(mu_layer, self.nb_obs, self.nb_action,self.input )#copy.deepcopy(self.actor)
        self.mu_prim.load_state_dict(self.actor.state_dict())
        self.Q_prim_1 = Critic(Q_layer, self.nb_obs, self.nb_action, self.input )#copy.deepcopy(self.critic_1)
        self.Q_prim_1.load_state_dict(self.critic_1.state_dict())
        self.Q_prim_2 = Critic(Q_layer, self.nb_obs, self.nb_action, self.input )#copy.deepcopy(self.critic_2)
