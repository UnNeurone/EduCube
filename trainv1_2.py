    ...
    def trainAgent(self, number_episode=50, number_timestep=100, stddev_Noise=0.2, render = True):
        ...
        for e in range(1, number_episode + 1):
            ...
            for t in tqdm(range(1, number_timestep + 1)):
                ...
                temp_action = self.mu(self.normalize_input(observation_t))
        ...
        filepath = "./models/"+self.env_name+"_"+str(year)+"_"+str(month)+"_"+str(day)+"_"+str(hour)+"_"+str(min)+"_Keras_DDPG/"

        self.Q.save(filepath+"_Q")
        self.Q_prim.save(filepath + "_Qprim")
...
if __name__ == '__main__':
    ...
    mu_layer = Q_layer#[400,300] #[256,256,256] #[256,256]

    Ad_alpha_Q = 1e-3#1e-3
    Ad_alpha_mu = 1e-4 #1e-4
    ...
    agent1 = DDPGAgent(env = envlist[1], activation=activationList[0], Q_layer=Q_layer,Ad_alpha_mu=Ad_alpha_mu,Ad_alpha_Q=Ad_alpha_Q, mu_layer=mu_layer,tau = tau,MiniBatch_size = MiniBatch_size,discount_gamma = discount_gamma)
